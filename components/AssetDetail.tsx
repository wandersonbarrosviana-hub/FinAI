...
const IndicatorCard = ({ label, value, suffix = '', prefix = '', color = 'text-slate-900 dark:text-white' }: { label: string, value: string | number, suffix?: string, prefix?: string, color?: string }) => (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between hover:shadow-md transition-all ring-1 ring-black/5 dark:ring-white/5">
        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest mb-3">{label}</span>
    ...
    return (
        <div className="animate-in slide-in-from-right duration-300">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-12">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${isFii ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400' : 'bg-sky-50 dark:bg-sky-900/20 text-sky-600 dark:text-sky-400'}`}>
                            {isFii ? 'FII' : 'Ação'}
                        </span>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className="text-slate-400 dark:text-slate-500 text-sm font-bold uppercase tracking-tight">{asset.name}</span>
                        <span className="w-1.5 h-1.5 rounded-full bg-slate-200 dark:bg-slate-700"></span>
                        <span className="text-slate-400 dark:text-slate-500 text-sm font-bold uppercase tracking-tight">{asset.segment}</span>
                    </div>
                </div>
                <div className="md:text-right bg-sky-50/50 dark:bg-sky-900/10 px-8 py-5 rounded-[2.5rem] border border-sky-100/50 dark:border-sky-900/20">
                    <p className="text-sky-600/50 dark:text-sky-400/50 text-[10px] font-black uppercase tracking-widest mb-1">Cotação Atual</p>
                    <p className="text-4xl font-black text-sky-600 dark:text-sky-400 tracking-tighter">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-12">
                <IndicatorCard label="Status Atual" value="Tempo Real" color="text-emerald-600 dark:text-emerald-400" />
                <IndicatorCard label="Dividend Yield" value={i.dy} suffix="%" color="text-emerald-600 dark:text-emerald-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                ...
                <div className="space-y-8">
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-8 shadow-sm ring-1 ring-black/5 dark:ring-white/5">
                        ...
                        <div className="space-y-6">
                            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100/50 dark:border-slate-700/50">
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest mb-1">Patrimônio Líquido</p>
                                <p className="text-slate-900 dark:text-white font-black tracking-tight text-lg">
                            ...
                            {!isFii && (
                                <>
                                    <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100/50 dark:border-slate-700/50">
                                        <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest mb-1">Dívida Líquida</p>
                                        <p className="text-slate-900 dark:text-white font-black tracking-tight text-lg">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-8 shadow-sm ring-1 ring-black/5 dark:ring-white/5">
                        <h3 className="text-sm font-black text-slate-900 dark:text-white mb-6 flex items-center gap-3 uppercase tracking-widest">
                            <Activity size={20} className="text-sky-500 dark:text-sky-400" />
                            Liquidez Diária
                        </h3>
                        <div className="p-4 bg-sky-50 dark:bg-sky-900/20 text-sky-600 dark:text-sky-400 rounded-2xl font-black text-xl tracking-tighter">
