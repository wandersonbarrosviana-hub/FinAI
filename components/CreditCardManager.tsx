            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Controle de limites e faturas</p>
                    </div>
                </div>
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
                    <button
                        onClick={() => { setActiveTab('list'); setSelectedCardId(null); }}
                        className={`px-4 py-2 rounded-lg text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'list' ? 'bg-white dark:bg-slate-700 text-sky-600 dark:text-sky-400 shadow-sm border border-slate-100 dark:border-slate-600' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'} `}
                    >
                        Cartões
                    </button>
                    <button
                        onClick={() => { setActiveTab('create'); setSelectedCardId(null); }}
                        className={`px-4 py-2 rounded-lg text-xs font-black transition-all uppercase tracking-widest ${activeTab === 'create' ? 'bg-white dark:bg-slate-700 text-sky-600 dark:text-sky-400 shadow-sm border border-slate-100 dark:border-slate-600' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'} `}
                    >
                        Novo Cartão
                    </button>
                </div>
                ...
                {activeTab === 'list' ? (
                    <div className="space-y-4">
                        {accounts.filter(a => a.isCredit).length === 0 ? (
                            <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 font-medium mb-4">Nenhum cartão de crédito cadastrado.</p>
                                <button
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                {accounts.filter(a => a.isCredit || a.type === 'credit').map(acc => {
                                    ...
                                    return (
                                        <div key={acc.id} className="relative group overflow-hidden bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 p-6 transition-all hover:shadow-xl hover:-translate-y-1">

                                            {/* Card Header */}
                                            <div className="flex justify-between items-start mb-6">
                                                <div className="flex items-center gap-3">
                                                    {bank?.logoUrl && (
                                                        <img src={bank.logoUrl} alt={bank.name} className="w-10 h-10 rounded-xl object-contain shadow-sm border border-slate-100 dark:border-slate-700 p-1 bg-white" />
                                                    )}
                                                    <div>
                                                        <h3 className="font-black text-slate-900 dark:text-white tracking-tight leading-tight">{acc.name}</h3>
                                            ...
                                            <div className="mb-6">
                                                <div className="flex justify-between text-xs font-bold mb-1">
                                                    ...
                                                    <span className="text-rose-500 dark:text-rose-400">R$ {currentInvoice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                                </div>
                                                ...
                                                <div className="flex justify-between text-[10px] font-bold mt-2 text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                <button
                                                    ...
                                                    className="flex items-center justify-center gap-2 py-3 bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-slate-200 dark:shadow-none uppercase tracking-widest"
                                                >
                                                    <Upload size={14} />
                                                    Importar
                                                </button>
                                                <button
                                                    ...
                                                    className="flex items-center justify-center gap-2 py-3 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold transition-all uppercase tracking-widest"
                                                >
                                                    Ver Fatura
                                                </button>
                                            </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                        <div className="flex items-center justify-between">
                            <button
                                onClick={() => { setActiveTab('list'); setSelectedCardId(null); }}
                                className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 font-bold transition-colors"
                            >
                                    ...
                                    {cardTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(t => (
                                        <div key={t.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 flex items-center justify-center shadow-sm text-slate-400 dark:text-slate-500 font-bold text-xs uppercase">
                                                    {new Date(t.date).getDate()}
                                                </div>
                                                <div>
                                                    <h4 className="font-bold text-slate-900 dark:text-white truncate max-w-[150px] sm:max-w-none">{t.description}</h4>
                                                    <p className="text-xs text-slate-500 dark:text-slate-400">{t.category} {t.installmentNumber ? `(${t.installmentNumber}/${t.installmentCount})` : ''}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="font-black text-slate-900 dark:text-white">R$ {Math.abs(t.amount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        ...
                        {!selectedBank ? (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="text-center mb-10">
                                    <div className="w-16 h-16 bg-sky-50 dark:bg-sky-900/10 rounded-full flex items-center justify-center mx-auto mb-4 text-sky-600 dark:text-sky-400">
                                        <Wallet size={32} />
                                    </div>
                                    <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Escolha a Instituição</h3>
                                    <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Selecione o emissor do seu cartão de crédito</p>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    {CARD_NETWORKS.map(bank => (
                                        <button
                                            ...
                                            className="flex flex-col items-center justify-center gap-4 p-6 bg-slate-50 dark:bg-slate-800 rounded-[2rem] border border-slate-200 dark:border-slate-700 hover:border-sky-500 dark:hover:border-sky-500 hover:bg-white dark:hover:bg-slate-800 hover:shadow-xl hover:shadow-sky-100 dark:hover:shadow-none transition-all group"
                                        >
                                            <div className="w-14 h-14 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center p-3 shadow-sm group-hover:scale-110 transition-all border border-slate-100 dark:border-slate-700">
                                                <img src={bank.logoUrl} alt={bank.name} className="w-full h-full object-contain" />
                                            </div>
                                            <span className="text-xs font-black text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white text-center uppercase tracking-tighter">{bank.name}</span>
                                        </button>
                                    ))}
                                    {/* Generic Option */}
                                    <button
                            ...
                            <div className="max-w-md mx-auto animate-in zoom-in-95 duration-300">
                                <div className="flex items-center gap-4 mb-8">
                                    <button onClick={() => setSelectedBank(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                                        <X size={20} />
                                    </button>

                                    {/* Header with bank logo and name input side-by-side */}
                                    <div className="flex-1 flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                                        {selectedBank.logoUrl && <img src={selectedBank.logoUrl} alt="" className="w-12 h-12 rounded-xl bg-white dark:bg-slate-900 p-2 border border-slate-100 dark:border-slate-700 shadow-sm object-contain flex-shrink-0" />}
                                        <div className="flex-1">
                                            <div className="flex flex-col">
                                                <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Nome do Cartão (Apelido)</span>
                                                <input
                                                    type="text"
                                                    value={cardName}
                                                    onChange={e => setCardName(e.target.value)}
                                                    className="w-full bg-transparent border-b-2 border-slate-200 dark:border-slate-600 focus:border-sky-500 dark:focus:border-sky-400 outline-none text-slate-900 dark:text-white font-black text-xl placeholder:text-slate-300 dark:placeholder:text-slate-600 transition-colors"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1 block pl-2">Limite Total</label>
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-10 pr-4 py-3 text-slate-700 dark:text-slate-200 font-bold focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 placeholder:font-normal"
                                                placeholder="0,00"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1 block pl-2">Dia Fechamento</label>
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-10 pr-4 py-3 text-slate-700 dark:text-slate-200 font-bold focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600"
                                                    placeholder="Dia"
                                                />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1 block pl-2">Dia Vencimento</label>
                                            <div className="relative">
                                                <Calendar size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                                                <input
                                                    type="number"
